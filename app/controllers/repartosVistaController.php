    ...
    try {
        ...
        if ($action === 'guardar_cambios_viaje') {
            ...
            try {
                ...
                if (empty($folio))
                    throw new Exception("El folio del viaje es requerido.");
                if ($vehiculo_id <= 0)
                    throw new Exception("Seleccione un vehículo válido.");
                if ($chofer_id <= 0)
                    throw new Exception("Debe asignar un chofer responsable.");
        ...
        if ($action === 'listar_pendientes_ruta') {
            ...
            $lista = $entregaM->listarSoloDespachadosPatioFecha(
                $almacen_id,
                $fecha_inicio,
                $fecha_fin
            );
