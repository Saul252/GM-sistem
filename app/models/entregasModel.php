...
class EntregaModel {
    ...
    public function procesarDespachoFisico($idMovimiento) {
        ...
        try {
            // 1. Obtener datos del movimiento y rastrear venta
            ...
            // 2. Buscar lotes disponibles (PEPS)
            ...
            if ($resLotes->num_rows == 0) {
                throw new Exception("No hay lotes disponibles en este almacén para el producto seleccionado.");
            }

            // 3. Afectación de lotes
            while ($cantidad_restante > 0 && $lote = $resLotes->fetch_assoc()) {
                ...
                $this->db->query("UPDATE lotes_stock SET 
                                    cantidad_actual = $nuevo_stock_lote, 
                ...
                if (!$this->db->query($sqlSalida)) {
                    throw new Exception("Error al insertar salida de lote: " . $this->db->error);
                }

                $cantidad_restante -= $a_tomar;
            }

            // 4. Registrar en tabla puente (Sincronizado con tu SQL de 2 usuarios)
            ...
            // Según tu tabla: necesitamos usuario_patio_id y usuario_despacho_id
...
// ... (dentro de la clase EntregaModel)

public function simularDespachoLotes($idMovimiento) {
    try {
        // Ajuste: Traemos el factor y la unidad de reporte desde el producto
...
public function obtenerDatosImpresion($idMovimiento) {
    ...
    if ($data) {
        // Cálculo de conversión para el total del ticket
