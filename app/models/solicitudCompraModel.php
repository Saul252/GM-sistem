...
class SolicitudCompra
{
    ...
    public function crear($data, $items)
    {
        try {
            ...
            $sqlCab = "INSERT INTO solicitudes_compra 
                   (administrador_id, almacen_id, proveedor_id, estado,ubicacion_id) 
                   VALUES (?, ?, ?, 'pendiente',?)";

            $stmt = $this->db->prepare($sqlCab);
            $stmt->bind_param("iiii", $data['usuario_id'], $data['almacen_id'], $data['proveedor_id'], $data['ubicacion_proveedor_id']);
    ...
    public function obtenerDetalle($id)
    {
        $sql = "SELECT d.*, p.nombre as producto_nombre, p.sku, p.unidad_medida, 
                   p.unidad_reporte, p.factor_conversion, s.almacen_id as almacen_origen_id,s.ubicacion_id as ubicacion_destino,
