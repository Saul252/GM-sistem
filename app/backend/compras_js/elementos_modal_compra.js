        ...
        if (tipo === 'compra') {
            html = `
                <div class="row g-2 mb-1 item-row align-items-end border-bottom pb-2" data-dist='[]'>
                    <div class="col-md-1 text-center"><button type="button" class="btn btn-sm btn-outline-success mb-1" onclick="modalProd.show()">+</button></div>
                    <div class="col-md-3">
                        <label class="small fw-bold">Producto</label>
                        <select class="form-select form-select-sm select-prod" required>
                            <option value="">Seleccione...</option>
                            <?php $productos_res->data_seek(0); while($p = $productos_res->fetch_assoc()): ?>
                                <option value="<?= $p['id'] ?>"><?= htmlspecialchars($p['nombre']) ?> (<?= $p['sku'] ?>)</option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    <div class="col-md-1">
                    ...
                    <div class="col-md-1 text-end"><button type="button" class="btn btn-link text-danger p-0 mb-1" onclick="$(this).closest('.row').remove(); calcularTotal();"><i class="bi bi-trash"></i></button></div>
