...
$filtro_folio = $_GET['folio_busqueda'] ?? '';
$filtro_usuario = $_GET['usuario_busqueda'] ?? '';

// Obtener lista de usuarios para el filtro select
$usuarios_res = $conexion->query("SELECT id, nombre FROM usuarios WHERE activo = 1 ORDER BY nombre ASC");
 ...
 <body>
             ...
             <div class="card-body bg-light">
                 <form method="GET" class="row g-3 align-items-end">
                     ...
                     <div class="col-md-2">
                         ...
                         <input type="text" name="folio_busqueda" class="form-control" placeholder="Buscar..."
    ...
    <thead>
        <tr>
            ...
            <th>Tipo</th>
            <th>Entidad</th>
            <th>Registrado Por</th> <th>Total Factura</th>
            <th>Estado</th>
    ...
    <tbody>
        ...
        $cond_c = "WHERE c.fecha_compra BETWEEN '$fecha_desde' AND '$fecha_hasta'";
        $cond_g = "WHERE g.fecha_gasto BETWEEN '$fecha_desde' AND '$fecha_hasta'";

        if($filtro_folio != '') {
            $cond_c .= " AND c.folio LIKE '%$filtro_folio%'";
            $cond_g .= " AND g.folio LIKE '%$filtro_folio%'";
        }
        if($filtro_usuario != '') {
            $cond_c .= " AND c.usuario_registra_id = '$filtro_usuario'";
            $cond_g .= " AND g.usuario_registra_id = '$filtro_usuario'";
        }

        $sqlEgresos = "(SELECT c.id, c.folio, c.proveedor as entidad, c.fecha_compra as fecha, c.total, 'compra' as tipo, c.tiene_faltantes, u.nombre as usuario_nombre 
                        FROM compras c LEFT JOIN usuarios u ON c.usuario_registra_id = u.id $cond_c)
                       UNION
                       (SELECT g.id, g.folio, g.beneficiario as entidad, g.fecha_gasto as fecha, g.total, 'gasto' as tipo, 0 as tiene_faltantes, u.nombre as usuario_nombre 
                        FROM gastos g LEFT JOIN usuarios u ON g.usuario_registra_id = u.id $cond_g)
                       ORDER BY folio DESC"; // Ordenado por Folio como pediste

        $resEgresos = $conexion->query($sqlEgresos);
        if(!$resEgresos) echo "Error: " . $conexion->error;
        ...
        <tr class="<?= $hayFaltantes ? 'table-warning' : '' ?>">
            ...
            <td><span class="badge rounded-pill <?= $e['tipo'] == 'compra' ? 'bg-primary' : 'bg-warning text-dark' ?>"><?= strtoupper($e['tipo']) ?></span></td>
            <td><?= htmlspecialchars($e['entidad']) ?></td>
            <td><small class="text-muted"><?= htmlspecialchars($e['usuario_nombre'] ?? 'N/A') ?></small></td>
            ...
            <td class="text-end">
                <div class="btn-group">
                    <button class="btn btn-sm btn-outline-dark" onclick="verDetalle('<?= $e['tipo'] ?>', <?= $e['id'] ?>)">Ver</button>
                    <?php if($hayFaltantes): ?>
