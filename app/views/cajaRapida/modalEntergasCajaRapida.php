...
$(document).ready(function() {
    // 1. Apuntamos al controlador que ya tiene la lógica de recursos y despacho
    const URL_CONTROLADOR = '/cfsistem/app/controllers/cajaRapidaController.php';
    ...
    window.prepararModalPatio = async function(movimientoId, almacenId) {
        ...
        try {
            // AJUSTE: Cambiamos 'ajax' por 'action' para que el IF del controlador lo detecte
            const [respDetalle, respRecursos] = await Promise.all([
                fetch(`${URL_CONTROLADOR}?action=get_recursos_reparto&id=${movimientoId}`),
                fetch(`${URL_CONTROLADOR}?action=get_recursos_sucursal&almacen_id=${almacenId}`)
            ]);
            ...
            if (resDetalle.success && resRecursos.success) {
                ...
                // AJUSTE: Usamos 'prod_nombre' que es el alias de tu consulta SQL
                $('#patio_producto_info').text(e.prod_nombre || e.producto_nombre || e.producto);
    ...
    /**
     * FUNCIÓN C: Envío del formulario de entrega en Patio
     */
    $('#formEntregaPatio').on('submit', async function(e) {
        ...
        try {
            ...
            // AJUSTE: Usamos 'action' para mantener el estándar del controlador
            formData.append('action', 'entregar_en_patio'); 

            const resp = await fetch(URL_CONTROLADOR, { 
                method: 'POST', 
                body: formData 
            });
            ...
            if (res.success || res.status === 'success') {
                ...
                Swal.fire({ 
                    ...
                    text: res.message || 'El ciclo de mercancía se ha cerrado correctamente.', 
                    timer: 2000 
                }).then(() => {
                    // Recargar tablas si existen funciones de renderizado
            ...
            console.error(error);
            Swal.fire('Error', 'Error crítico al procesar la entrega.', 'error');
        } finally {
