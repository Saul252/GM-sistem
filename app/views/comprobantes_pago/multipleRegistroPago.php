...
<div class="modal fade" id="modalDispersión" tabindex="-1" aria-labelledby="modalDispersiónLabel" aria-hidden="true">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-4">
            <div class="card card-total text-center border-0 bg-primary-soft p-2">
        ...
        <div class="col-4">
            <div class="card card-total text-center border-0 bg-success-soft p-2">
    ...
    <div>
        ...
        <div class="table-responsive border rounded-3 bg-white shadow-sm">
            ...
            <!-- Pie del Modal -->
            <div class="modal-footer border-0 bg-light py-3 px-4" style="border-bottom-left-radius: 16px; border-bottom-right-radius: 16px;">
...
<script>
   ...
   async function getDeuda(id, monto,idComprobante) {
    ...
    try {
        const res = await fetch(`/cfsistem/app/controllers/ventasHistorialController.php?${params.toString()}`);
        ...
        ventasAplicadas.forEach(v => {
            contenedorRenglones.innerHTML += `
                <tr>
                    ...
                    <td class="text-center">
                        <button type="button" class="btn btn-sm btn-outline-danger border-0 rounded-2" onclick="quitarVenta('${v.id}')">
                            ✕ Quitar
                        </button>
                    </td>
                </tr>
...
async function guardarDispersiónPagos() {
    ...
    try {
        ...
        for (let i = 0; i < datosPagos.ids.length; i++) {
            ...
            const res = await fetch(`/cfsistem/app/controllers/registrarPagosController.php?action=guardarAbono`, { 
                method: 'POST', 
                body: fd 
            });
        ...
        fd2.append('cantidadAplicada',monto)
          ...
          const resp = await fetch(`/cfsistem/app/controllers/comprobantesPagoController.php?action=actualizarAplicado`, {
                method: 'POST',
                body: fd2
            });
